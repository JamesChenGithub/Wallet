...
- (NSData *)rawMetaData
{
    if (_resultMetadata)
    {
        NSArray *bytesValue = _resultMetadata[@(kResultMetadataTypeByteSegments)];

        if (bytesValue)
        {
            NSArray *bytes = bytesValue[0];
            
            if (bytes)
            {
                Byte bytesArray[bytes.count];
                int i = 0;
                for (NSNumber *num in bytes)
                {
                    bytesArray[i++] = num.intValue;
                }
                
                NSData *rawData = [NSData dataWithBytes:&bytesArray length:bytes.count];
                return rawData;
            }
        }
